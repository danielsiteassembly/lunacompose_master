...
final class VL_License_Manager {
    ...
    public static function lic_lookup_by_key($key) {
        if (self::is_legacy_license_key($key)) {
            return null;
        }
...
<?php endforeach; ?>
    ...
    /**
     * Forces cookie SameSite attribute to None for cross-subdomain access.
     */
    public function force_cookie_samesite_none($samesite, $cookie_name, $user_id) {
        return 'None';
    }
    
    /**
     * Forces cookie domain to .visiblelight.ai for cross-subdomain access.
     */
    public function force_cookie_domain($domain) {
        return '.visiblelight.ai';
    }
    ...
    private function add_cors_headers() {
        // Only add CORS headers for REST API requests
        if (headers_sent()) {
            return;
        }
        
        // Check if request is from supercluster subdomain or visiblelight.ai domain
        $origin = isset($_SERVER['HTTP_ORIGIN']) ? $_SERVER['HTTP_ORIGIN'] : '';
        $referer = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '';
        
        // Allow requests from supercluster.visiblelight.ai or visiblelight.ai
        ...
        // Also check referer if origin is not set
        if (!$is_allowed_origin && !empty($referer)) {
            foreach ($allowed_origins as $allowed) {
        ...
        // If no origin/referer check, allow for REST API requests
        if (!$is_allowed_origin && (defined('REST_REQUEST') && REST_REQUEST)) {
            $is_allowed_origin = true;
        }
    ...
    public function register_rest_routes() {
        ...
        // Category-specific endpoints for Luna Chat Widget
        // These endpoints return VL Hub data filtered by category
        $category_endpoints = array(
            'security' => array('security', 'cloudops'),
            'connections' => array('infrastructure', 'cloudops', 'security', 'analytics', 'search', 'marketing', 'ecommerce'),
            'cloudops' => array('cloudops', 'infrastructure'),
            ...
            'plugins' => array('security', 'infrastructure'),
            'themes' => array('security', 'infrastructure'),
            'updates' => array('infrastructure', 'security'),
            'web-infra' => array('infrastructure', 'cloudops'),
            'web-infrastructure' => array('infrastructure', 'cloudops'),
            'infra' => array('infrastructure', 'cloudops'),
            'cloud-ops' => array('cloudops'),
            'all-connections' => array('infrastructure', 'cloudops', 'security', 'analytics', 'search', 'marketing', 'ecommerce', 'identity', 'competitive'),
            'data-sources' => array('infrastructure', 'cloudops', 'security', 'analytics', 'search', 'marketing', 'ecommerce', 'identity', 'competitive'),
            'search-console' => array('search'),
            'ga4' => array('analytics'),
            'e-commerce' => array('ecommerce'),
            'competition' => array('competitive'),
            'competitors' => array('competitive'),
        );
        
        foreach ($category_endpoints as $endpoint => $categories) {
            register_rest_route(
                'vl-hub/v1',
                '/' . $endpoint,
                array(
                    'methods' => 'GET',
                    'permission_callback' => '__return_true',
                    'callback' => array($this, 'rest_category_data_wrapper'),
                )
            );
        }
    ...
    /**
     * REST handler wrapper: extracts endpoint from request and calls rest_category_data.
     * 
     * @param WP_REST_Request $request The REST request
     * @return WP_REST_Response
     */
    public function rest_category_data_wrapper($request) {
        // Extract endpoint from request path
        $route = $request->get_route();
        $endpoint = str_replace('/vl-hub/v1/', '', $route);
        ...
        // Get categories for this endpoint
        $categories = isset($category_endpoints[$endpoint]) ? $category_endpoints[$endpoint] : array();
        
        return $this->rest_category_data($request, $categories);
    }
    
    /**
     * REST handler: return category-specific data from VL Hub.
     * 
     * @param WP_REST_Request $request The REST request
     * @param array $categories Array of category slugs to include
     * @return WP_REST_Response
     */
    public function rest_category_data($request, $categories) {
        ...
        // Filter streams by requested categories
        $category_streams = array();
        foreach ($license_streams as $stream_id => $stream) {
            if (isset($stream['categories']) && is_array($stream['categories'])) {
                foreach ($stream['categories'] as $stream_category) {
                    if (in_array($stream_category, $categories, true)) {
                        $category_streams[$stream_id] = $stream;
                        break;
                    }
                }
            }
        ...
        $category_data = array(
            ...
            'data_streams' => $category_streams,
            'stream_count' => count($category_streams),
        );
        
        // Add category-specific VL Hub data
        foreach ($categories as $category) {
            switch ($category) {
                case 'security':
                    // SSL/TLS Status
                    $ssl_tls_settings = get_option('vl_ssl_tls_settings_' . $license_key, array());
                    if (!empty($ssl_tls_settings)) {
                        $category_data['ssl_tls'] = array(
                            'certificate' => $ssl_tls_settings['certificate'] ?? '',
                            'issuer' => $ssl_tls_settings['issuer'] ?? '',
                            'expires' => $ssl_tls_settings['expires'] ?? '',
                            'days_until_expiry' => $ssl_tls_settings['days_until_expiry'] ?? null,
                            'connected' => isset($ssl_tls_settings['connected']) ? (bool)$ssl_tls_settings['connected'] : false,
                        );
                    }
                    
                    // Cloudflare Data
                    $cloudflare_settings = get_option('vl_cloudflare_settings_' . $license_key, array());
                    $cloudflare_zones = get_option('vl_cloudflare_zones_' . $license_key, array());
                    if (!empty($cloudflare_settings)) {
                        $category_data['cloudflare'] = array(
                            'connected' => !empty($cloudflare_settings['api_token']) && !empty($cloudflare_settings['account_id']),
                            'account_id' => $cloudflare_settings['account_id'] ?? '',
                            'zones_count' => $cloudflare_settings['zones_count'] ?? count($cloudflare_zones),
                            'zones' => $cloudflare_zones,
                            'last_sync' => $cloudflare_settings['last_sync'] ?? null,
                        );
                    }
                    break;
                ...
                case 'infrastructure':
                    // AWS S3 Data
                    $aws_s3_settings = get_option('vl_aws_s3_settings_' . $license_key, array());
                    $aws_s3_buckets = get_option('vl_aws_s3_buckets_' . $license_key, array());
                    if (!empty($aws_s3_settings)) {
                        $category_data['aws_s3'] = array(
                            'connected' => isset($aws_s3_settings['connected']) ? (bool)$aws_s3_settings['connected'] : false,
                            'bucket_count' => $aws_s3_settings['bucket_count'] ?? count($aws_s3_buckets),
                            'object_count' => $aws_s3_settings['object_count'] ?? 0,
                            'storage_used' => $aws_s3_settings['storage_used'] ?? '0 B',
                            'buckets' => $aws_s3_buckets,
                            'last_sync' => $aws_s3_settings['last_sync'] ?? null,
                        );
                    }
                    
                    // Liquid Web Assets
                    $liquidweb_settings = get_option('vl_liquidweb_settings_' . $license_key, array());
                    $liquidweb_assets = get_option('vl_liquidweb_assets_' . $license_key, array());
                    if (!empty($liquidweb_settings)) {
                        $category_data['liquidweb'] = array(
                            'connected' => !empty($liquidweb_settings['api_key']) && !empty($liquidweb_settings['account_number']),
                            'account_number' => $liquidweb_settings['account_number'] ?? '',
                            'asset_count' => count($liquidweb_assets),
                            'assets' => $liquidweb_assets,
                            'last_sync' => $liquidweb_settings['last_sync'] ?? null,
                        );
                    }
                    break;
                ...
                case 'analytics':
                    // GA4 Data
                    $all_streams_data = self::data_streams_store_get();
                    if (!empty($all_streams_data[$license_key])) {
                        foreach ($all_streams_data[$license_key] as $stream_id => $stream_data) {
                            if (!empty($stream_data['ga4_metrics'])) {
                                $category_data['ga4'] = array(
                                    'property_id' => $stream_data['ga4_property_id'] ?? '',
                                    'measurement_id' => $stream_data['ga4_measurement_id'] ?? '',
                                    'metrics' => $stream_data['ga4_metrics'] ?? array(),
                                    'last_synced' => $stream_data['ga4_last_synced'] ?? $stream_data['last_updated'] ?? null,
                                    'date_range' => $stream_data['ga4_date_range'] ?? array(),
                                    'source_url' => $stream_data['source_url'] ?? null,
                                );
                                break;
                            }
                        }
                ...
                case 'search':
                    ...
                    if (!empty($gsc_settings)) {
                        $category_data['gsc'] = array(
                            'connected' => !empty($gsc_settings['service_account_json']) && !empty($gsc_settings['site_url']),
                            'site_url' => $gsc_settings['site_url'] ?? '',
                            'data' => $gsc_data,
                            'last_sync' => $gsc_settings['last_sync'] ?? null,
                        );
                    }
                    break;
                ...
                case 'competitive':
                    // Competitor Reports
                    global $wpdb;
                    $competitor_reports_table = $wpdb->prefix . 'vl_competitor_reports';
                    $reports = $wpdb->get_results($wpdb->prepare(
                        "SELECT * FROM {$competitor_reports_table} WHERE license_key = %s ORDER BY last_scanned DESC",
                        $license_key
                    ), ARRAY_A);
                    if (!empty($reports)) {
                        $category_data['competitor_reports'] = array();
                        foreach ($reports as $report) {
                            $report_data = maybe_unserialize($report['report_json']) ?: json_decode($report['report_json'], true);
                            $category_data['competitor_reports'][] = array(
                                'url' => $report['competitor_url'],
                                'domain' => parse_url($report['competitor_url'], PHP_URL_HOST),
                    ...
                    // VLDR Metrics
                    $vldr_table = $wpdb->prefix . 'vl_competitor_metrics';
                    $vldr_metrics = $wpdb->get_results($wpdb->prepare(
                        "SELECT m1.* FROM {$vldr_table} m1
                        INNER JOIN (
                            SELECT domain, MAX(metric_date) as max_date
                            FROM {$vldr_table}
                            WHERE license_key = %s
                            GROUP BY domain
                        ) m2 ON m1.domain = m2.domain AND m1.metric_date = m2.max_date
                        WHERE m1.license_key = %s
                        ORDER BY m1.vldr_score DESC",
                        $license_key,
                        $license_key
                    ), ARRAY_A);
                    if (!empty($vldr_metrics)) {
                        $category_data['vldr_metrics'] = array();
                        foreach ($vldr_metrics as $metric) {
                            $category_data['vldr_metrics'][$metric['domain']] = array(
                                'domain' => $metric['domain'],
                                'vldr_score' => floatval($metric['vldr_score']),
                                'ref_domains' => intval($metric['ref_domains']),
                                'indexed_pages' => intval($metric['indexed_pages']),
                                'lighthouse_avg' => intval($metric['lighthouse_avg']),
                                'security_grade' => $metric['security_grade'],
                                'domain_age_years' => floatval($metric['domain_age_years']),
                                'uptime_percent' => floatval($metric['uptime_percent']),
                ...
                case 'content':
                    // WordPress Content Data (from client site)
                    if (!empty($license_record['site'])) {
                        ...
                        if ($posts_data || $pages_data) {
                            $category_data['wordpress_content'] = array(
                ...
                case 'identity':
                    // WordPress Users Data (from client site)
                    if (!empty($license_record['site'])) {
                        $users_data = self::fetch_client_wp_data($license_key, 'users');
                        if ($users_data) {
                            $category_data['wordpress_users'] = array(
        ...
        // Apply filter to enrich category data
        $category_data = apply_filters('vl_hub_category_data', $category_data, $license_key, $categories);
    ...
    public function rest_profile($request) {
        ...
        $profile = array(
            ...
            'site_info' => array(
                ...
                'luna_supercluster_shortcode' => isset($license_record['luna_supercluster_shortcode']) ? $license_record['luna_supercluster_shortcode'] : '[luna_chat vl_key=' . $license_key . ']',
            ),
            'wordpress' => array(),
            'security' => array(),
            'content' => array(),
        ...
        // Add ALL VL Hub data sources to profile
        // AWS S3 Data
        $aws_s3_data = get_option('vl_aws_s3_data_' . $license_key, array());
        $aws_s3_buckets = get_option('vl_aws_s3_buckets_' . $license_key, array());
        $aws_s3_settings = get_option('vl_aws_s3_settings_' . $license_key, array());
        if (!empty($aws_s3_data) || !empty($aws_s3_buckets) || !empty($aws_s3_settings)) {
            $profile['aws_s3'] = array(
                'settings' => $aws_s3_settings,
                'buckets' => $aws_s3_buckets,
                'data' => $aws_s3_data,
            );
        }
        ...
        $liquidweb_assets = get_option('vl_liquidweb_assets_' . $license_key, array());
        $liquidweb_settings = get_option('vl_liquidweb_settings_' . $license_key, array());
        if (!empty($liquidweb_assets) || !empty($liquidweb_settings)) {
            $profile['liquidweb'] = array(
                'settings' => $liquidweb_settings,
                'assets' => $liquidweb_assets,
            );
        }
        
        // GA4 Data (from data streams)
        $all_streams = get_option('vl_data_streams', array());
        if (!empty($all_streams[$license_key])) {
            foreach ($all_streams[$license_key] as $stream_id => $stream_data) {
                if (!empty($stream_data['ga4_metrics'])) {
                    $profile['ga4_metrics'] = $stream_data['ga4_metrics'];
                    $profile['ga4_last_synced'] = $stream_data['ga4_last_synced'] ?? $stream_data['last_updated'] ?? null;
                    $profile['ga4_date_range'] = $stream_data['ga4_date_range'] ?? null;
                    $profile['ga4_source_url'] = $stream_data['source_url'] ?? null;
                    $profile['ga4_property_id'] = $stream_data['ga4_property_id'] ?? null;
                    $profile['ga4_measurement_id'] = $stream_data['ga4_measurement_id'] ?? null;
        ...
        // All Data Streams
        if (!empty($all_streams[$license_key])) {
            $profile['data_streams'] = $all_streams[$license_key];
        }
        
        // Competitor Reports (full reports)
        global $wpdb;
        $competitor_reports_table = $wpdb->prefix . 'vl_competitor_reports';
        $reports = $wpdb->get_results($wpdb->prepare(
            "SELECT * FROM {$competitor_reports_table} WHERE license_key = %s ORDER BY last_scanned DESC",
        ...
        if (!empty($reports)) {
            $profile['competitor_reports_full'] = array();
            foreach ($reports as $report) {
                $report_data = maybe_unserialize($report['report_json']) ?: json_decode($report['report_json'], true);
                $profile['competitor_reports_full'][] = array(
                    'url' => $report['competitor_url'],
        ...
        // VLDR Metrics (latest for all domains)
        $vldr_table = $wpdb->prefix . 'vl_competitor_metrics';
        $vldr_metrics = $wpdb->get_results($wpdb->prepare(
            "SELECT m1.* FROM {$vldr_table} m1
            INNER JOIN (
        ...
        if (!empty($vldr_metrics)) {
            $profile['vldr_metrics'] = array();
            foreach ($vldr_metrics as $metric) {
                $profile['vldr_metrics'][$metric['domain']] = array(
                    'domain' => $metric['domain'],
                    'vldr_score' => floatval($metric['vldr_score']),
                    'ref_domains' => intval($metric['ref_domains']),
                    'indexed_pages' => intval($metric['indexed_pages']),
        ...
        $ssl_tls_settings = get_option('vl_ssl_tls_settings_' . $license_key, array());
        if (!empty($ssl_tls_settings)) {
            $profile['ssl_tls'] = array(
                'certificate' => $ssl_tls_settings['certificate'] ?? '',
                'issuer' => $ssl_tls_settings['issuer'] ?? '',
                'expires' => $ssl_tls_settings['expires'] ?? '',
                'days_until_expiry' => $ssl_tls_settings['days_until_expiry'] ?? null,
                'connected' => isset($ssl_tls_settings['connected']) ? (bool)$ssl_tls_settings['connected'] : false,
                'status' => $ssl_tls_settings['status'] ?? 'unknown',
                'last_checked' => $ssl_tls_settings['last_checked'] ?? null,
            );
        }
        ...
        $cloudflare_zones = get_option('vl_cloudflare_zones_' . $license_key, array());
        if (!empty($cloudflare_settings)) {
            $profile['cloudflare'] = array(
                'connected' => !empty($cloudflare_settings['api_token']) && !empty($cloudflare_settings['account_id']),
                'account_id' => $cloudflare_settings['account_id'] ?? '',
                'api_token_set' => !empty($cloudflare_settings['api_token']),
                'zone_id' => $cloudflare_settings['zone_id'] ?? null,
                'last_sync' => $cloudflare_settings['last_sync'] ?? null,
                'zones_count' => $cloudflare_settings['zones_count'] ?? count($cloudflare_zones),
                'zones' => $cloudflare_zones,
            );
    ...
    /**
     * Syncs Cloudflare data and creates data streams.
     * 
     * @param string $license_key The license key
     * @return array|WP_Error Sync result with zones_count and streams_created, or error
     */
    public static function sync_cloudflare_data($license_key) {
        ...
        try {
            ...
            $zones_response = self::cloudflare_api_handler($license_key, 'zones', array('per_page' => 50));
            ...
            if (isset($zones_response['result']) && is_array($zones_response['result'])) {
                ...
                foreach ($zones as $zone) {
                    ...
                    $zone_plan = isset($zone['plan']) && is_array($zone['plan']) ? ($zone['plan']['name'] ?? 'Free') : 'Free';
                    ...
                    // Create data stream for each zone with "security" category
                    $stream_id = 'cloudflare_zone_' . md5($zone_id);
                    $health_score = $zone_status === 'active' ? 95 : ($zone_status === 'pending' ? 80 : 60);
                    
                    $stream_data = array(
                        'name' => 'Cloudflare: ' . esc_html($zone_name),
                        'description' => 'Cloudflare zone with DDoS protection, WAF, and CDN features',
                        'categories' => array('security', 'cloudops'),
                        'health_score' => $health_score,
                        'error_count' => $zone_status === 'active' ? 0 : 1,
                        'warning_count' => $zone_status === 'pending' ? 1 : 0,
                        'status' => $zone_status === 'active' ? 'active' : 'inactive',
                        'cloudflare_zone_id' => $zone_id,
                        'cloudflare_zone_name' => $zone_name,
                        'cloudflare_plan' => $zone_plan,
                        'cloudflare_data' => $zone,
                        'last_updated' => current_time('mysql'),
                    );
            ...
            $settings['zones_count'] = $zones_count;
            $settings['last_sync'] = current_time('mysql');
            $settings['zones_data'] = $zones_data;
            update_option('vl_cloudflare_settings_' . $license_key, $settings);
            
            // Also store zones data separately for profile endpoint access
            update_option('vl_cloudflare_zones_' . $license_key, $zones_data);
            ...
            error_log('[VL Hub] Cloudflare sync exception: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
            return new WP_Error('cloudflare_sync_error', 'Cloudflare sync failed: ' . $e->getMessage());
        }
    }
     ...
     */
    public static function check_ssl_tls_status($license_key) {
        $license = self::lic_lookup_by_key($license_key);
        ...
        try {
            ...
            // Use curl as primary method for getting SSL certificate (more reliable)
            if (function_exists('curl_init')) {
                ...
                curl_exec($ch);
                $cert_info_array = curl_getinfo($ch, CURLINFO_CERTINFO);
                $curl_error = curl_error($ch);
                curl_close($ch);
                
                if (!empty($cert_info_array) && is_array($cert_info_array) && !empty($cert_info_array[0])) {
                    // Parse certificate info from curl
                    $cert_data = $cert_info_array[0];
                    if (isset($cert_data['Cert'])) {
                        $cert_pem = $cert_data['Cert'];
                        $cert = openssl_x509_read($cert_pem);
                    }
                } elseif (!empty($curl_error)) {
                    $ssl_status['error'] = "Curl SSL check failed: {$curl_error}";
                    return $ssl_status;
                }
            }
            
            // Fallback: Use stream socket if curl didn't work
            if (!$cert) {
            ...
            if (!$socket) {
                ...
                // Send a simple HTTP request to complete the handshake
                fwrite($socket, "GET / HTTP/1.0\r\nHost: {$host}\r\n\r\n");
                fread($socket, 1); // Read at least one byte to ensure handshake completes
                
                // Get certificate information from context
                $context_params = stream_context_get_params($context);
                
                // Try multiple ways to get the certificate
                if (isset($context_params['options']['ssl']['peer_certificate'])) {
                    $cert = $context_params['options']['ssl']['peer_certificate'];
                } elseif (function_exists('stream_context_get_options')) {
                    // Try to get certificate using stream_context_get_options
                    $g = stream_context_get_options($context);
                    if (isset($g['ssl']['peer_certificate'])) {
                        $cert = $g['ssl']['peer_certificate'];
                    }
                }
            ...
            if ($cert) {
                $cert_info = openssl_x509_parse($cert, false);
                if ($cert_info && isset($cert_info['validFrom_time_t']) && isset($cert_info['validTo_time_t'])) {
                ...
                $ssl_status['connected'] = true;
                    ...
                    } elseif (isset($cert_info['subject']['commonName'])) {
                        $subject_cn = $cert_info['subject']['commonName'];
                    }
                    
                    // Get issuer CN
                    $issuer_cn = 'Unknown';
                    if (isset($cert_info['issuer']['CN'])) {
                        $issuer_cn = $cert_info['issuer']['CN'];
                    } elseif (isset($cert_info['issuer']['commonName'])) {
                        $issuer_cn = $cert_info['issuer']['commonName'];
                    } elseif (isset($cert_info['issuer']) && is_array($cert_info['issuer'])) {
                        // Try to get any issuer name
                        $issuer_cn = isset($cert_info['issuer']['O']) ? $cert_info['issuer']['O'] : (isset($cert_info['issuer']['OU']) ? $cert_info['issuer']['OU'] : 'Unknown');
                    }
                ...
                $ssl_status['cipher_suite'] = 'Secure';
                    ...
                    $ssl_status['error'] = 'Could not parse certificate information';
                    error_log('[VL Hub] SSL/TLS certificate parse failed. cert_info keys: ' . (is_array($cert_info) ? implode(', ', array_keys($cert_info)) : 'not array'));
                }
            } else {
                $ssl_status['error'] = 'Could not retrieve certificate from SSL connection';
                // Log debug info if available
                if (isset($context_params) && is_array($context_params) && isset($context_params['options'])) {
                    error_log('[VL Hub] SSL/TLS certificate not found in context params. Available keys: ' . implode(', ', array_keys($context_params['options'])));
                } else {
                    error_log('[VL Hub] SSL/TLS certificate not found - context params not available');
                }
            }
        ...
        // Also store in WordPress option for profile endpoint access
        if ($ssl_status['connected'] && !empty($ssl_status['certificate_info']) && is_array($ssl_status['certificate_info'])) {
            $cert_info = $ssl_status['certificate_info'];
            $ssl_tls_settings = array(
                'certificate' => isset($cert_info['subject']) ? $cert_info['subject'] : '',
                'issuer' => isset($cert_info['issuer']) ? $cert_info['issuer'] : '',
                'expires' => isset($cert_info['valid_to']) ? $cert_info['valid_to'] : '',
                'expires_at' => isset($cert_info['valid_to']) ? $cert_info['valid_to'] : '',
                'days_until_expiry' => isset($ssl_status['expiry_days']) ? $ssl_status['expiry_days'] : null,
                'connected' => true,
                'status' => 'active',
                'valid_from' => isset($cert_info['valid_from']) ? $cert_info['valid_from'] : '',
                'last_checked' => current_time('mysql'),
                'tls_version' => isset($ssl_status['tls_version']) ? $ssl_status['tls_version'] : null,
                'cipher_suite' => isset($ssl_status['cipher_suite']) ? $ssl_status['cipher_suite'] : null,
            );
            update_option('vl_ssl_tls_settings_' . $license_key, $ssl_tls_settings);
            
            // Create/update data stream for SSL/TLS Status in Security category
            $health_score = 100;
            if (isset($ssl_status['expiry_days']) && $ssl_status['expiry_days'] < 30) {
                $health_score = $ssl_status['expiry_days'] < 7 ? 60 : 80;
            }
            if (isset($ssl_status['error']) && !empty($ssl_status['error'])) {
                $health_score = 70;
            }
            
            self::add_data_stream($license_key, 'ssl_tls_status', array(
                'name' => 'SSL/TLS Status',
                'description' => 'SSL certificate monitoring and expiry alerts',
                'categories' => array('security'),
                'health_score' => $health_score,
                'error_count' => ((isset($ssl_status['expiry_days']) && $ssl_status['expiry_days'] < 7) || isset($ssl_status['error'])) ? 1 : 0,
                'warning_count' => (isset($ssl_status['expiry_days']) && $ssl_status['expiry_days'] < 30 && $ssl_status['expiry_days'] >= 7) ? 1 : 0,
                'status' => 'active',
                'ssl_tls_data' => $ssl_tls_settings,
                'last_updated' => current_time('mysql'),
            ));
        }
    ...
    public static function render_security_connections($license_key, $license) {
        ...
        // Check real SSL/TLS status - DISABLED during admin page loads to prevent blocking
        // The SSL check will be performed via AJAX when the user clicks "Connect" or "Recheck"
        $ssl_tls_status = array('connected' => false, 'certificate_info' => null, 'expiry_days' => null, 'error' => null);
        $ssl_tls_connected = false;
        
        // Try to get cached SSL status if available (from previous checks)
        $license = self::lic_lookup_by_key($license_key);
        if ($license && !empty($license['site'])) {
            $site_url = rtrim($license['site'], '/');
            $parsed_url = parse_url($site_url);
            ...
            if ($cached_status !== false && is_array($cached_status)) {
                $ssl_tls_status = $cached_status;
                $ssl_tls_connected = isset($ssl_tls_status['connected']) ? (bool)$ssl_tls_status['connected'] : false;
            }
        }
        ...
        foreach ($connections as $connection) {
            ...
            if ($connection['name'] === 'SSL/TLS Status' && isset($ssl_tls_status['certificate_info']) && !empty($ssl_tls_status['certificate_info'])) {
                $cert_info = $ssl_tls_status['certificate_info'];
                $expiry_days = isset($ssl_tls_status['expiry_days']) ? $ssl_tls_status['expiry_days'] : null;
                $expiry_color = ($expiry_days !== null && $expiry_days > 30) ? '#00a32a' : (($expiry_days !== null && $expiry_days > 7) ? '#dba617' : '#d63638');
                ...
                $html .= '<div style="color: ' . $expiry_color . '; font-weight: bold;">';
                    $html .= '<strong>Days until expiry:</strong> ' . $expiry_days . ' days';
                $html .= '</div>';
                }
                if (isset($ssl_tls_status['error']) && !empty($ssl_tls_status['error'])) {
            ...
            // Add proper onclick handlers for each connection type
            if ($connection['name'] === 'SSL/TLS Status') {
                if ($ssl_tls_connected) {
                    $html .= '<button type="button" class="button button-secondary" onclick="syncSSLTLSStatus(\'' . esc_js($license_key) . '\')" style="font-size: 12px; padding: 5px 10px;">Recheck</button>';
                } else {
                    $html .= '<button type="button" class="button button-primary" onclick="syncSSLTLSStatus(\'' . esc_js($license_key) . '\')" style="font-size: 12px; padding: 5px 10px;">Connect</button>';
                }
            } elseif ($connection['name'] === 'Cloudflare') {
                if ($cloudflare_connected) {
                    $html .= '<button type="button" class="button button-secondary" onclick="manageCloudflare(\'' . esc_js($license_key) . '\')" style="font-size: 12px; padding: 5px 10px;">Manage</button>';
                    $html .= '<button type="button" class="button button-secondary" onclick="syncCloudflare(\'' . esc_js($license_key) . '\')" style="font-size: 12px; padding: 5px 10px;">Sync Now</button>';
                } else {
                    $html .= '<button type="button" class="button button-primary" onclick="manageCloudflare(\'' . esc_js($license_key) . '\')" style="font-size: 12px; padding: 5px 10px;">Connect</button>';
                }
            } else {
            $html .= '<button type="button" class="button button-primary" style="font-size: 12px; padding: 5px 10px;">Connect</button>';
            }
        ...
        // Add JavaScript for SSL/TLS Status and Cloudflare sync
        $html .= '<script type="text/javascript">';
        $html .= 'function syncSSLTLSStatus(licenseKey) {';
        $html .= 'var button = event.target;';
        $html .= 'var originalText = button.textContent;';
        $html .= 'button.disabled = true;';
        $html .= 'button.textContent = "Checking...";';
        $html .= 'jQuery.ajax({';
        $html .= 'url: "' . admin_url('admin-ajax.php') . '",';
        $html .= 'type: "POST",';
        $html .= 'data: {';
        $html .= 'action: "vl_sync_ssl_tls_status",';
        $html .= 'license_key: licenseKey,';
        $html .= 'nonce: "' . wp_create_nonce('vl_ssl_tls_nonce') . '"';
        $html .= '},';
        $html .= 'success: function(response) {';
        $html .= 'if (response.success) {';
        $html .= 'alert("SSL/TLS Status checked successfully! Certificate: " + (response.data.certificate || "Unknown") + "\\nIssuer: " + (response.data.issuer || "Unknown") + "\\nExpires: " + (response.data.expires || "Unknown") + "\\nDays until expiry: " + (response.data.days_until_expiry || "N/A"));';
        $html .= 'location.reload();';
        $html .= '} else {';
        $html .= 'alert("Error checking SSL/TLS status: " + (response.data || "Unknown error"));';
        $html .= 'button.disabled = false;';
        $html .= 'button.textContent = originalText;';
        $html .= '}';
        $html .= '},';
        $html .= 'error: function() {';
        $html .= 'alert("Error checking SSL/TLS status. Please try again.");';
        $html .= 'button.disabled = false;';
        $html .= 'button.textContent = originalText;';
        $html .= '}';
        $html .= '});';
        $html .= '}';
        $html .= 'function manageCloudflare(licenseKey) {';
        $html .= 'var dataStreamsTab = jQuery("a[href*=data-streams], a[href*=data-stream], .nav-tab[data-tab=data-streams]");';
        $html .= 'if (dataStreamsTab.length) {';
        $html .= 'dataStreamsTab.trigger("click");';
        $html .= 'setTimeout(function() {';
        $html .= 'var categoryFilter = jQuery("#stream-category-filter, select[name*=category], .stream-category-filter");';
        $html .= 'if (categoryFilter.length) {';
        $html .= 'categoryFilter.val("security").trigger("change");';
        $html .= '}';
        $html .= 'jQuery("html, body").animate({ scrollTop: 0 }, 300);';
        $html .= '}, 500);';
        $html .= '} else {';
        $html .= 'var dataStreamsSection = jQuery("#data-streams, .data-streams-section, [data-section=data-streams]");';
        $html .= 'if (dataStreamsSection.length) {';
        $html .= 'jQuery("html, body").animate({ scrollTop: dataStreamsSection.offset().top - 100 }, 500);';
        $html .= 'var categoryFilter = jQuery("#stream-category-filter");';
        $html .= 'if (categoryFilter.length) {';
        $html .= 'categoryFilter.val("security").trigger("change");';
        $html .= '}';
        $html .= '} else {';
        $html .= 'alert("Please navigate to the Data Streams section to view Cloudflare connections.");';
        $html .= '}';
        $html .= '}';
        $html .= '}';
        $html .= 'function syncCloudflare(licenseKey) {';
        $html .= 'var button = event.target;';
        $html .= 'var originalText = button.textContent;';
        $html .= 'button.disabled = true;';
        $html .= 'button.textContent = "Syncing...";';
        $html .= 'jQuery.ajax({';
        $html .= 'url: "' . admin_url('admin-ajax.php') . '",';
        $html .= 'type: "POST",';
        $html .= 'data: {';
        $html .= 'action: "vl_sync_cloudflare",';
        $html .= 'license_key: licenseKey,';
        $html .= 'nonce: "' . wp_create_nonce('vl_cloudflare_nonce') . '"';
        $html .= '},';
        $html .= 'success: function(response) {';
        $html .= 'if (response.success) {';
        $html .= 'alert("Cloudflare data synced successfully! " + (response.data.message || ""));';
        $html .= 'location.reload();';
        $html .= '} else {';
        $html .= 'alert("Error syncing Cloudflare: " + (response.data || "Unknown error"));';
        $html .= 'button.disabled = false;';
        $html .= 'button.textContent = originalText;';
        $html .= '}';
        $html .= '},';
        $html .= 'error: function() {';
        $html .= 'alert("Error syncing Cloudflare. Please try again.");';
        $html .= 'button.disabled = false;';
        $html .= 'button.textContent = originalText;';
        $html .= '}';
        $html .= '});';
    ...
    public function render_client_edit_screen($license_key, $license, $messages) {
        ...
        if (isset($_POST['action'])) {
            ...
            if ('update_luna_supercluster' === $action) {
                check_admin_referer('vl_update_luna_supercluster');
                
                $shortcode = isset($_POST['luna_supercluster_shortcode']) ? trim(wp_unslash($_POST['luna_supercluster_shortcode'])) : '';
                
                $store = self::lic_store_get();
                if (isset($store[$license_key])) {
                    $store[$license_key]['luna_supercluster_shortcode'] = $shortcode;
                    $store[$license_key]['last_updated'] = current_time('mysql');
                    
                    self::lic_store_set($store);
                    $messages['success'][] = 'Luna to Supercluster shortcode updated successfully.';
    ...
    public function render_hub_profile_screen() {
        ...
        // Handle Luna to Supercluster shortcode form submission
        if (isset($_POST['action']) && $_POST['action'] === 'update_luna_supercluster') {
            check_admin_referer('vl_update_luna_supercluster');
            
            $license_key = isset($_POST['license_key']) ? sanitize_text_field(wp_unslash($_POST['license_key'])) : '';
            $shortcode = isset($_POST['luna_supercluster_shortcode']) ? trim(wp_unslash($_POST['luna_supercluster_shortcode'])) : '';
            
            if ($license_key && isset($licenses[$license_key])) {
                $store = self::lic_store_get();
                if (isset($store[$license_key])) {
                    $store[$license_key]['luna_supercluster_shortcode'] = $shortcode;
                    $store[$license_key]['last_updated'] = current_time('mysql');
                    ...
                    // Update licenses array for display
                    $licenses = $store;
                    if ($selected_license === $license_key) {
                        $selected_license = $license_key; // Refresh selected license
                    }
                }
            }
        }
            ...
            <div class="vl-hub-tabs" style="margin-top: 20px;">
                ...
                <?php if ($selected_license && isset($licenses[$selected_license])) : ?>
                    ...
                    <div class="vl-client-profile">
                        ...
                        <!-- Luna to Supercluster -->
                        <div style="margin-top: 20px; padding: 15px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 5px;">
                            <h3 style="margin-top: 0;">Luna to Supercluster</h3>
                            <form method="post" id="luna-supercluster-form">
                                <?php wp_nonce_field('vl_update_luna_supercluster'); ?>
                                <input type="hidden" name="action" value="update_luna_supercluster">
                                <input type="hidden" name="license_key" value="<?php echo esc_attr($selected_license); ?>">
                                
                                <table class="form-table">
                                    <tr>
                                        <th scope="row"><label for="luna_supercluster_shortcode">Luna Chat Shortcode</label></th>
                                        <td>
                                            <input type="text" id="luna_supercluster_shortcode" name="luna_supercluster_shortcode" 
                                                   value="<?php echo esc_attr(isset($license['luna_supercluster_shortcode']) ? $license['luna_supercluster_shortcode'] : '[luna_chat vl_key=' . $selected_license . ']'); ?>" 
                                                   class="regular-text code" placeholder="[luna_chat vl_key=VL-XXXX-XXXX-XXXX]" />
                                            <p class="description">
                                                Enter the Luna Chat shortcode. Default: <code>[luna_chat vl_key=<?php echo esc_html($selected_license); ?>]</code><br>
                                                This shortcode will be used to display the Luna Chat Widget in the Supercluster dashboard.
                                            </p>
                                        </td>
                                    </tr>
                                </table>
                                ...
                                <p class="submit">
                                    <input type="submit" class="button-primary" value="Save Shortcode" />
                                </p>
                            </form>
                        </div>
...
// AJAX handler for syncing SSL/TLS status
add_action('wp_ajax_vl_sync_ssl_tls_status', function() {
    try {
        check_ajax_referer('vl_ssl_tls_nonce', 'nonce');
    } catch (Exception $e) {
        error_log('[VL Hub] SSL/TLS sync nonce check failed: ' . $e->getMessage());
    ...
    try {
        ...
        // Check SSL/TLS status (this will cache and store in option)
        $ssl_status = VL_License_Manager::check_ssl_tls_status($license_key);
        
        if ($ssl_status['connected'] && !empty($ssl_status['certificate_info'])) {
            $ssl_tls_settings = get_option('vl_ssl_tls_settings_' . $license_key, array());
            wp_send_json_success(array(
                'certificate' => $ssl_tls_settings['certificate'] ?? $ssl_status['certificate_info']['subject'],
                'issuer' => $ssl_tls_settings['issuer'] ?? $ssl_status['certificate_info']['issuer'],
                'expires' => $ssl_tls_settings['expires'] ?? $ssl_status['certificate_info']['valid_to'],
                'days_until_expiry' => $ssl_tls_settings['days_until_expiry'] ?? $ssl_status['expiry_days'],
                'connected' => true
            ));
        } else {
            $error_message = $ssl_status['error'] ?? 'SSL/TLS check failed';
            error_log('[VL Hub] SSL/TLS check failed for license ' . $license_key . ': ' . $error_message);
            wp_send_json_error($error_message);
        }
    } catch (Exception $e) {
        error_log('[VL Hub] SSL/TLS sync exception: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
        wp_send_json_error('SSL/TLS check failed: ' . $e->getMessage());
    } catch (Error $e) {
        error_log('[VL Hub] SSL/TLS sync fatal error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
        wp_send_json_error('SSL/TLS check failed: ' . $e->getMessage());
    }
});

// AJAX handler for syncing Cloudflare data
add_action('wp_ajax_vl_sync_cloudflare', function() {
    try {
        check_ajax_referer('vl_cloudflare_nonce', 'nonce');
    } catch (Exception $e) {
        error_log('[VL Hub] Cloudflare sync nonce check failed: ' . $e->getMessage());
        wp_send_json_error('Nonce verification failed');
        return;
    }
    ...
    try {
        ...
        if (empty($settings['api_token']) || empty($settings['account_id'])) {
            wp_send_json_error('Cloudflare not connected. Please configure API token and Account ID first.');
            return;
        }
        
        // Sync Cloudflare data
        if (!method_exists('VL_License_Manager', 'sync_cloudflare_data')) {
            wp_send_json_error('Cloudflare sync function not available');
            return;
        }
        
        $sync_result = VL_License_Manager::sync_cloudflare_data($license_key);
        ...
        if (!is_array($sync_result)) {
            wp_send_json_error('Invalid sync result format');
            return;
        }
        
        wp_send_json_success(array(
            'message' => 'Cloudflare data synced successfully',
            'zones_count' => $sync_result['zones_count'] ?? 0,
            'streams_created' => $sync_result['streams_created'] ?? 0,
        ));
    } catch (Exception $e) {
        error_log('[VL Hub] Cloudflare sync exception: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
        wp_send_json_error('Cloudflare sync failed: ' . $e->getMessage());
    } catch (Error $e) {
        error_log('[VL Hub] Cloudflare sync fatal error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
        wp_send_json_error('Cloudflare sync failed: ' . $e->getMessage());
    }
});
